...
from ui_purchaseDialog import Ui_ViewOrderDialog
...
class EmployeeDashboard(QWidget):
    def __init__(self, parent=None):
        ...
        self.ui.setupUi(self)
        self.ui.products.horizontalHeader().setVisible(True)
        self.ui.products.horizontalHeader().setCascadingSectionResizes(False)
        self.ui.products.horizontalHeader().setHighlightSections(False)
        self.ui.products.horizontalHeader().setStretchLastSection(False)
        self.ui.products.verticalHeader().setVisible(False)
        self.ui.products.verticalHeader().setHighlightSections(False)
        self.login = LoginPage()
        self.ui.logOutButton.clicked.connect(lambda: (self.close(), self.login.show()))
        self.ui.purchaseButton.clicked.connect(self.order_product)
        self.product_order = []
        self.product_table()

    def order_product(self):
        self.view_order = ViewOrder(self.product_order)
        self.view_order.exec()
    ...
    def product_table(self):
        ...
        for row_number, row_data in enumerate(self.result):
            ...
            self.order_details_button = EmployeeButton(row_number, row_data, self)
            for column_number, column_data in enumerate(row_data):
                self.ui.products.setItem(row_number, column_number, QTableWidgetItem(str(column_data)))
                self.ui.products.setCellWidget(row_number, 5, self.order_details_button)
                self.ui.products.setRowHeight(row_number, 50)

        self.connection.close()

class ViewOrder(QDialog):
    def __init__(self, product_ordered,parent=None):
        super().__init__(parent)
        self.ui = Ui_ViewOrderDialog()
        self.ui.setupUi(self)
        self.product_ordered = product_ordered
        inventory_header = self.ui.cartTable.horizontalHeader()
        inventory_header.setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.load_view_order()
    
    def load_view_order(self):
        self.ui.cartTable.setRowCount(0)
        for row_number, row_data in enumerate(self.product_ordered):
...
class EmployeeButton(QWidget):
    ...
    def __init__(self, row_number, row_data, manager):
        ...
        self.order_button.setStyleSheet("background-color: blue;")
        self.order_button.setFixedSize(61,31)
        self.order_button.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        self.order_button.clicked.connect(self.order_dialog)
        layout.addWidget(self.details_button)
        layout.addWidget(self.order_button)

    
    def order_dialog(self):
        self.manager.product_order.append(self.row_data)
        self.order_button.setEnabled(False)
        print(self.manager.product_order)

    def details_dialog(self):
        self.details_item = DetailDialog(self.row_data)
        self.details_item.data_update.connect(self.manager.product_table)
